...
var _setSpecies = function(KEY){
  var C = typeof core$2[KEY] == 'function' ? core$2[KEY] : global$9[KEY];
...
$export$4($export$4.G + $export$4.W + $export$4.F * !USE_NATIVE$1, {Promise: $Promise});
_setToStringTag($Promise, PROMISE);
_setSpecies(PROMISE);
Wrapper = _core[PROMISE];

// statics
$export$4($export$4.S + $export$4.F * !USE_NATIVE$1, PROMISE, {
...
$export$4($export$4.S + $export$4.F * (LIBRARY$2 || !USE_NATIVE$1), PROMISE, {
...
$export$4($export$4.S + $export$4.F * !(USE_NATIVE$1 && _iterDetect(function(iter){
...
function collect$1(f, xs) {
    return fold$1(function (acc, x) { return append$1(acc, f(x)); }, new List(), xs);
}
...
function parse(input, actuallyParse, url) {
    ...
    }(actuallyParse(new Chunks(new List(), ofArray(split$$1(url, "/"))))(input));
}
...
function format$$1(input, parse_1, chunks, fmt) {
...
setType("Fable.PowerPack.Result.Result", Result$1);

function map$3(fn, a) {
    return a.Case === "Error" ? new Result$1("Error", [a.Fields[0]]) : new Result$1("Ok", [fn(a.Fields[0])]);
}
function bind(fn, a) {
    return a.Case === "Error" ? new Result$1("Error", [a.Fields[0]]) : fn(a.Fields[0]);
}
...
var _Promise$1 = function (__exports) {
    ...
    var bindResult = __exports.bindResult = function bindResult(fn, a) {
        return a.then(function (a_1) {
            return a_1.Case === "Error" ? Promise.resolve(new Result$1("Error", [a_1.Fields[0]])) : result$$1(fn(a_1.Fields[0]));
        });
    };
...
function create$5(ie, comparer) {
    comparer = comparer || new GenericComparer();
    return from$5(comparer, ie ? tree_ofSeq$1(comparer, ie) : new SetTree("SetEmpty", []));
}
...
var Program$1 = function (__exports) {
    var toNavigable = __exports.toNavigable = function (parser, urlUpdate, program) {
        ...
        var update = function update(msg) {
            return function (model) {
                return map$$1(msg.Case === "UserMsg" ? program.update(msg.Fields[0])(model) : urlUpdate(parser(msg.Fields[0]))(model));
            };
        };

        var locationChanges = function locationChanges(dispatch) {
            window.addEventListener('popstate', function (ev) {
                return dispatch(new Navigable("Change", [window.location]));
            });
            window.addEventListener("NavigatedEvent", function (ev) {
                return dispatch(new Navigable("Change", [window.location]));
            });
        };
        ...
        return new Program(init, update, subs, function (model) {
            return function (dispatch) {
...
var Model = function () {
    function Model(page, query, cache) {
...
function toHash(_arg1) {
    if (_arg1.Case === "Blog") {
        return "#blog/" + String(_arg1.Fields[0]);
    } else if (_arg1.Case === "Search") {
        return "#search/" + _arg1.Fields[0];
    } else {
        return "#home";
    }
}
var pageParser = oneOf(ofArray([function () {
    var input = new Page("Home", []);

    var parse$$1 = function () {
        var str$$1 = "home";
        return function (arg10_) {
            return function (result) {
                return s(str$$1, arg10_, result);
            };
        };
    }();
    ...
    return function (chunks) {
        return function (fmt) {
            return format$$1(input, parse$$1, chunks, fmt);
        };
    };
}(), function () {
    var input = function input(arg0) {
        return new Page("Blog", [arg0]);
    };
    ...
    var parse$$1 = function () {
        ...
        var parseRest = function parseRest(chunks) {
            return i32(chunks);
        };

        return function (chunks) {
            return function (fmt) {
                return op_LessDivideGreater(parseFirst, parseRest, chunks, fmt);
            };
        };
    }();
...
function hashParser(location) {
    return parse(function (x) {
        return x;
    }, pageParser, location.hash.substr(1));
}
var Msg = function () {
    ...
    _createClass(Msg, [{
        ...
        value: function () {
            return {
                ...
                cases: {
                    Enter: [],
                    FetchFailure: ["string", Error],
                    FetchSuccess: ["string", makeGeneric(List, {
                        T: "string"
                    })],
                    Query: ["string"]
                }
...
setType("App.Msg", Msg);
var Place = function () {
    function Place(place_name, state) {
    ...
    _createClass(Place, [{
        ...
        value: function () {
            ...
            return {
                type: "App.Place",
                interfaces: ["FSharpRecord", "System.IEquatable", "System.IComparable"],
                properties: (_properties = {}, _defineProperty(_properties, "place name", "string"), _defineProperty(_properties, "state", "string"), _properties)
            };
        }
    }, {
        key: "Equals",
...
setType("App.Place", Place);
var ZipResponse = function () {
    function ZipResponse(places) {
        _classCallCheck(this, ZipResponse);

        this.places = places;
    }

    _createClass(ZipResponse, [{
        key: _Symbol.reflection,
        value: function () {
            return {
                type: "App.ZipResponse",
                interfaces: ["FSharpRecord", "System.IEquatable", "System.IComparable"],
                properties: {
                    places: makeGeneric(List, {
                        T: Place
                    })
                }
            };
...
setType("App.ZipResponse", ZipResponse);
function get(query) {
    return function (builder_) {
        return builder_.Delay(function () {
            return fetchAs("http://api.zippopotam.us/us/" + query, {}, {
                T: ZipResponse
            }).then(function (_arg1) {
                return _Promise.resolve(function (r) {
                    return map$2(function (p) {
                        return p["place name"] + ", " + p.state;
                    }, r.places);
                }(_arg1));
            });
        });
...
function urlUpdate(result, model) {
    if (result.Case === "Ok") {
        if (result.Fields[0].Case === "Search") {
            var _ret = function () {
                var page = result.Fields[0];
                var query = result.Fields[0].Fields[0];
                return {
                    v: [new Model(page, query, model.cache), model.cache.has(query) ? new List() : CmdModule.ofPromise(function (query_1) {
                        return get(query_1);
                    }, query, function (r) {
                        return new Msg("FetchSuccess", [query, r]);
                    }, function (ex) {
                        return new Msg("FetchFailure", [query, ex]);
                    })]
                };
            }();
            ...
            var page = result.Fields[0];
            return [new Model(page, "", model.cache), new List()];
        }
    } else {
        var e = result.Fields[0];
        console.error("Error parsing url:", e);
        return [model, Navigation.modifyUrl(toHash(model.page))];
    }
}
function init(result) {
    return urlUpdate(result, new Model(new Page("Home", []), "", create$1(null, new GenericComparer(compare))));
}
function update(msg, model) {
    if (msg.Case === "Enter") {
        var newPage = new Page("Search", [model.query]);
        return [new Model(newPage, model.query, model.cache), Navigation.newUrl(toHash(newPage))];
    } else if (msg.Case === "FetchFailure") {
        return [function () {
            var cache = add(msg.Fields[0], new List(), model.cache);
            return new Model(model.page, model.query, cache);
        }(), new List()];
    } else if (msg.Case === "FetchSuccess") {
        return [function () {
            var cache = add(msg.Fields[0], msg.Fields[1], model.cache);
            return new Model(model.page, model.query, cache);
        }(), new List()];
    } else {
        return [new Model(model.page, msg.Fields[0], model.cache), new List()];
    }
}
...
function words(size, message) {
    return react.createElement("span", {
        style: {
            fontSize: fsFormat("%dpx")(function (x) {
                return x;
            })(size)
        }
    }, message);
...
function onEnter(msg, dispatch) {
    return ["onKeyDown", function (_arg1) {
        if (_arg1.keyCode === 13) {
...
function viewPage(model, dispatch) {
    if (model.page.Case === "Blog") {
        return ofArray([words(20, "This is blog post number"), words(100, String(model.page.Fields[0]))]);
    } else if (model.page.Case === "Search") {
        var matchValue = tryFind$$1(model.page.Fields[0], model.cache);

        if (matchValue != null) {
            if (matchValue.tail != null) {
                var location = matchValue.head;
                return ofArray([words(20, "Zip code " + model.page.Fields[0] + " is in " + location + "!")]);
            } else {
                return ofArray(["No results found for " + model.page.Fields[0] + ". Need a valid zip code like 90210."]);
            }
        } else {
            return ofArray(["..."]);
        }
    } else {
        return ofArray([words(60, "Welcome!?"), "Play with the links and search bar above. (Press ENTER to trigger the zip code search.)"]);
    }
}
function view(model, dispatch) {
    return react.createElement("div", {}, react.createElement("div", fold$1(function (o, kv) {
        o[kv[0]] = kv[1];
        return o;
    }, {}, [centerStyle("row")]), viewLink(new Page("Home", []), "Home"), viewLink(new Page("Blog", [42]), "Cat Facts"), viewLink(new Page("Blog", [13]), "Alligator Jokes"), viewLink(new Page("Blog", [26]), "Workout Plan"), react.createElement("input", fold$1(function (o, kv) {
        o[kv[0]] = kv[1];
        return o;
    }, {}, [["style", {
        width: "200px",
        ...
        dispatch(new Msg("Query", [ev.target.value]));
    }], onEnter(new Msg("Enter", []), dispatch), ["value", model.query], ["placeholder", "Enter a zip code (e.g. 90210)"]]))), react.createElement("hr", {}), react.createElement.apply(undefined, ["div", fold$1(function (o, kv) {
        o[kv[0]] = kv[1];
        return o;
    }, {}, [centerStyle("column")])].concat(_toConsumableArray(viewPage(model, dispatch)))));
}
ProgramModule.run(withReact("elmish-app", ProgramModule.withConsoleTrace(function () {
    var parser = function parser(location) {
        return hashParser(location);
    };

    return function (program) {
        return Program$1.toNavigable(parser, function (result) {
            return function (model) {
                return urlUpdate(result, model);
            };
        }, program);
    };
}()(ProgramModule.mkProgram(function (result) {
    return init(result);
}, function (msg) {
    return function (model) {
...
}((this.Client = this.Client || {}),React,ReactDOM));
